...
build() {
    ...
    if [[ $app =~ $valid_url ]]; then
        if ! curl --output applications/app.zip --silent --fail "${app}"; then
            echo "The given URL or Path to the Cyb3rhq Apps is not working: ${app}"
            clean 1
        else
        ...
        echo "The given URL or Path to the Cyb3rhq App is not valid: ${app}"
        clean 1
    fi
    ...
    if [[ $base =~ $valid_url ]]; then
        if [[ $base =~ .*\.zip ]]; then
            if ! curl --output cyb3rhq-dashboard.zip --silent --fail "${base}"; then
                echo "The given URL or Path to the Cyb3rhq Dashboard base is not working: ${base}"
                clean 1
            else
                echo "Extracting Cyb3rhq Dashboard base"
                unzip -q cyb3rhq-dashboard.zip -d ./dashboards/
                rm cyb3rhq-dashboard.zip
                mv ./dashboards/$(ls ./dashboards) cyb3rhq-dashboard.tar.gz
            fi
        else
            if ! curl --output cyb3rhq-dashboard.tar.gz --silent --fail "${base}"; then
                echo "The given URL or Path to the Cyb3rhq Dashboard base is not working: ${base}"
                clean 1
            fi
        fi
    else
        echo "The given URL or Path to the Cyb3rhq Dashboard base is not valid: ${base}"
        clean 1
    fi
    ...
    if [[ $security =~ $valid_url ]]; then
        if ! curl --output applications/security.zip --silent --fail "${security}"; then
            echo "The given URL or Path to the Cyb3rhq Security Plugin is not working: ${security}"
            clean 1
        else
        ...
        echo "The given URL or Path to the Cyb3rhq Security Plugin is not valid: ${security}"
        clean 1
    fi

    tar -zxf cyb3rhq-dashboard.tar.gz
    directory_name=$(ls -td */ | head -1)
    working_dir="cyb3rhq-dashboard-$version-$revision-linux-x64"
    ...
    # Install Cyb3rhq apps and Security app
...
help() {
    ...
    echo "    -a, --app <url/path>          Set the location of the .zip file containing the Cyb3rhq plugin."
    echo "    -b, --base <url/path>         Set the location of the .tar.gz file containing the base cyb3rhq-dashboard build."
    echo "    -s, --security <url/path>     Set the location of the .zip file containing the cyb3rhq-security-dashboards-plugin."
